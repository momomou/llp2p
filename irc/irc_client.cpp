...
int irc_client_thread (void *arc)
{
	...
	irc_cli_thread *irc_cli_thread_ptr = (irc_cli_thread *)arc;

	unsigned short port = irc_cli_thread_ptr->port;
	string ip = irc_cli_thread_ptr->ip;
	string nick = irc_cli_thread_ptr->nick;
	string channel = irc_cli_thread_ptr->channel;
	...
	if ( irc_connect (s, ip.c_str(), port, 0, nick.c_str(), 0, 0) )
	{
