 ...
 */
 ...
 /*
 *    the whole file is copied from librtmp and flvstreamer
 *    may not keep the coding style
 */
...
unsigned short amf::AMF_DecodeInt16(const char *data)
{
...
unsigned int amf::AMF_DecodeInt24(const char *data)
{
	unsigned char *c = (unsigned char *) data;
	unsigned int val;
...
unsigned int amf::AMF_DecodeInt32(const char *data)
{
	unsigned char *c = (unsigned char *) data;
	unsigned int val;
...
void amf::AMF_DecodeString(const char *data, AVal * bv)
{
...
bool amf::AMF_DecodeBoolean(const char *data)
{
	return *data != 0;
}

char * amf::AMF_EncodeInt16(char *output, char *outend, short nVal)
{
	if (output+2 > outend)
		return NULL;
...
char * amf::AMF_EncodeInt24(char *output, char *outend, int nVal)
{
	if (output+3 > outend)
		return NULL;
...
char * amf::AMF_EncodeInt32(char *output, char *outend, int nVal)
{
	if (output+4 > outend)
		return NULL;
...
char * amf::AMF_EncodeNumber(char *output, char *outend, double dVal)
{
...
char * amf::AMF_EncodeBoolean(char *output, char *outend, bool bVal)
{
	if (output+2 > outend)
		return NULL;
...
void amf::AMFProp_GetName(AMFObjectProperty * prop, AVal * name)
{
	*name = prop->p_name;
}

void amf::AMFProp_SetName(AMFObjectProperty * prop, AVal * name)
{
	prop->p_name = *name;
}

AMFDataType amf::AMFProp_GetType(AMFObjectProperty * prop)
{
	return prop->p_type;
}

double amf::AMFProp_GetNumber(AMFObjectProperty * prop)
{
	return prop->p_vu.p_number;
}

int amf::AMFProp_GetBoolean(AMFObjectProperty * prop)
{
	return prop->p_vu.p_number != 0;
}

void amf::AMFProp_GetString(AMFObjectProperty * prop, AVal * str)
{
	*str = prop->p_vu.p_aval;
}

void amf::AMFProp_GetObject(AMFObjectProperty * prop, AMFObject * obj)
{
	*obj = prop->p_vu.p_object;
}

int amf::AMFProp_IsValid(AMFObjectProperty * prop)
{
	return prop->p_type != AMF_INVALID;
}

char * amf::AMFProp_Encode(AMFObjectProperty * prop, char *pBuffer, char *pBufEnd)
{
	...
	switch (prop->p_type){
		...
		default:
			_log_ptr->Log(LOGERROR, "%s, invalid type. %d", __FUNCTION__, prop->p_type);
			pBuffer = NULL;
		};
...
int amf::AMF3ReadInteger(const char *data, int32_t * valp)
{
	int i = 0;
	int32_t val = 0;
...
int amf::AMF3ReadString(const char *data, AVal * str)
{
	assert(str != 0);
	...
	if ((ref & 0x1) == 0) {				/* reference: 0xxx */
		...
		_log_ptr->Log(LOGDEBUG, "%s, string reference, index: %d, not supported, ignoring!", refIndex);
		return len;
	} else {
...
int amf::AMF3Prop_Decode(AMFObjectProperty * prop, const char *pBuffer, int nSize,
		int bDecodeName)
{
	...
	if (nSize == 0 || !pBuffer) {
		_log_ptr->Log(LOGDEBUG, "empty buffer/no buffer pointer!");
		return -1;
	}
	...
	switch (type) {
		...
		case AMF3_DATE:
		{
			...
			if ((res & 0x1) == 0) {			/* reference */
				...
				_log_ptr->Log(LOGDEBUG, "AMF3_DATE reference: %d, not supported!", nIndex);
			} else {
				if (nSize < 8)
					return -1;
		...
		default:
			_log_ptr->Log(LOGDEBUG, "%s - AMF3 unknown/unsupported datatype 0x%02x, @0x%08X",
...
int amf::AMFProp_Decode(AMFObjectProperty * prop, const char *pBuffer, int nSize,
				 int bDecodeName)
{
	int nOriginalSize = nSize;
	...
	if (nSize == 0 || !pBuffer) {
		_log_ptr->Log(LOGDEBUG, "%s: Empty buffer/no buffer pointer!", __FUNCTION__);
		return -1;
	}

	if (bDecodeName && nSize < 4) {				/* at least name (length + at least 1 byte) and 1 byte of data */
		_log_ptr->Log(LOGDEBUG, "%s: Not enough data for decoding with name, less then 4 bytes!", __FUNCTION__);
		return -1;
	}
	...
	if (bDecodeName) {
		...
		if (nNameSize > nSize - 2) {
			_log_ptr->Log(LOGDEBUG, "%s: Name size out of range: namesize (%d) > len (%d) - 2",
	...
	switch (prop->p_type) {
		...
		case AMF_MOVIECLIP:
		{
			_log_ptr->Log(LOGERROR, "AMF_MOVIECLIP reserved!");
			return -1;
			break;
		}
		case AMF_NULL:
		...
		case AMF_REFERENCE:
		{
			_log_ptr->Log(LOGERROR, "AMF_REFERENCE not supported!");
			return -1;
			break;
		}
		case AMF_ECMA_ARRAY:
		...
		case AMF_DATE:
		{
			_log_ptr->Log(LOGDEBUG, "AMF_DATE");
		...
		case AMF_RECORDSET:
		{
			_log_ptr->Log(LOGERROR, "AMF_RECORDSET reserved!");
			return -1;
			break;
		}
		case AMF_XML_DOC:
		{
			_log_ptr->Log(LOGERROR, "AMF_XML_DOC not supported!");
			return -1;
			break;
		}
		case AMF_TYPED_OBJECT:
		{
			_log_ptr->Log(LOGERROR, "AMF_TYPED_OBJECT not supported!");
			return -1;
			break;
		}
		case AMF_AVMPLUS:
		...
		default:
			_log_ptr->Log(LOGDEBUG, "%s - unknown datatype 0x%02x, @0x%08X", __FUNCTION__,
...
void amf::AMFProp_Dump(AMFObjectProperty * prop)
{
	char strRes[256];
	char str[256];
	AVal name;
	...
	if (prop->p_type == AMF_INVALID) {
		_log_ptr->Log(LOGDEBUG, "Property: INVALID");
		return;
	}

	if (prop->p_type == AMF_NULL) {
		_log_ptr->Log(LOGDEBUG, "Property: NULL");
		return;
	}
...
#ifdef _WIN32
	_snprintf(strRes, 255, "Name: %25.*s, ", name.av_len, name.av_val);
#else
	snprintf(strRes, 255, "Name: %25.*s, ", name.av_len, name.av_val);
#endif

	if (prop->p_type == AMF_OBJECT) {
		_log_ptr->Log(LOGDEBUG, "Property: <%sOBJECT>", strRes);
	...
	switch (prop->p_type) {
		...
		case AMF_BOOLEAN:
#ifdef _WIN32
			_snprintf(str, 255, "BOOLEAN:\t%s", prop->p_vu.p_number != 0.0 ? "TRUE" : "FALSE");
#else
		...
		case AMF_STRING:
#ifdef _WIN32
			_snprintf(str, 255, "STRING:\t%.*s", prop->p_vu.p_aval.av_len, prop->p_vu.p_aval.av_val);
#else
		...
		case AMF_DATE:
#ifdef _WIN32
			_snprintf(str, 255, "DATE:\ttimestamp: %.2f, UTC offset: %d", prop->p_vu.p_number, prop->p_UTCoffset);
#else
	...
	_log_ptr->Log(LOGDEBUG, "Property: <%s%s>", strRes, str);
}

void amf::AMFProp_Reset(AMFObjectProperty * prop)
{
	if (prop->p_type == AMF_OBJECT) {
...
char * amf::AMF_Encode(AMFObject * obj, char *pBuffer, char *pBufEnd)
{
	int i;
	...
	*pBuffer++ = AMF_OBJECT;
	...
	for (i = 0; i < obj->o_num; i++) {
		...
		if (res == NULL) {
			_log_ptr->Log(LOGERROR, "AMF_Encode - failed to encode property in index %d", i);
			break;
		} else {
			pBuffer = res;
		}
...
int amf::AMF_DecodeArray(AMFObject * obj, const char *pBuffer, int nSize,
...
int amf::AMF3_Decode(AMFObject * obj, const char *pBuffer, int nSize, bool bAMFData)
{
	int nOriginalSize = nSize;
	int32_t ref;
	int len;
	...
	if (bAMFData) {
		if (*pBuffer != AMF3_OBJECT)
			_log_ptr->Log(LOGERROR, "AMF3 Object encapsulated in AMF stream does not start with AMF3_OBJECT!");
		pBuffer++;
		nSize--;
	}
	...
	if ((ref & 1) == 0) {				/* object reference, 0xxx */
			...
			_log_ptr->Log(LOGDEBUG, "Object reference, index: %d", objectIndex);
		...
		if ((classRef & 0x1) == 0) {			/* class reference */
			...
			_log_ptr->Log(LOGDEBUG, "Class reference: %d", classIndex);
		} else {
			...
			_log_ptr->Log(LOGDEBUG, "Class name: %s, externalizable: %d, dynamic: %d, classMembers: %d",
			...
			for (i = 0; i < cd.cd_num; i++) {
				...
				_log_ptr->Log(LOGDEBUG, "Member: %s", memberName.av_val);
		...
		if (cd.cd_externalizable) {
			...
			_log_ptr->Log(LOGDEBUG, "Externalizable, TODO check");

			nRes = AMF3Prop_Decode(&prop, pBuffer, nSize, false);
			if (nRes == -1) {
				_log_ptr->Log(LOGDEBUG, "%s, failed to decode AMF3 property!",
					__FUNCTION__);
			} else {
				nSize -= nRes;
				pBuffer += nRes;
			...
			for (i = 0; i < cd.cd_num; i++) {	/* non-dynamic */
				nRes = AMF3Prop_Decode(&prop, pBuffer, nSize, false);
				if (nRes == -1)
					_log_ptr->Log(LOGDEBUG, "%s, failed to decode AMF3 property!", __FUNCTION__);
		...
		_log_ptr->Log(LOGDEBUG, "class object!");
	}
	return nOriginalSize - nSize;
}

int amf::AMF_Decode(AMFObject * obj, const char *pBuffer, int nSize, bool bDecodeName)
{
	int nOriginalSize = nSize;
...
void amf::AMF_AddProp(AMFObject * obj, const AMFObjectProperty * prop)
{
	if (!(obj->o_num & 0x0f))
		obj->o_props =
...
int amf::AMF_CountProp(AMFObject * obj)
{
	return obj->o_num;
}

AMFObjectProperty * amf::AMF_GetProp(AMFObject * obj, const AVal * name, int nIndex)
{
	if (nIndex >= 0) {
		if (nIndex <= obj->o_num)
...
void amf::AMF_Dump(AMFObject * obj)
{
	int n;
	_log_ptr->Log(LOGDEBUG, "(object begin)");
	for (n = 0; n < obj->o_num; n++) {
		AMFProp_Dump(&obj->o_props[n]);
	}
	_log_ptr->Log(LOGDEBUG, "(object end)");
}

void amf::AMF_Reset(AMFObject * obj)
{
	int n;
	for (n = 0; n < obj->o_num; n++) {
...
void amf::AMF3CD_AddProp(AMF3ClassDef * cd, AVal * prop)
{
	if (!(cd->cd_num & 0x0f))
...
AVal * amf::AMF3CD_GetProp(AMF3ClassDef * cd, int nIndex)
{
	if (nIndex >= cd->cd_num)
