...
io_accept::io_accept(network *net_ptr,logger *log_ptr,configuration *prep_ptr,peer_mgr * peer_mgr_ptr,peer *peer_ptr,pk_mgr * pk_mgr_ptr, peer_communication *peer_communication_ptr){
	_net_ptr = net_ptr;
	_log_ptr = log_ptr;
	_prep = prep_ptr;
...
{	
	...
	*/
	...
	if(new_fd < 0) {
		...
		while (1) {
			...
			if (recv_byte < 0) {
				...
				if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
					...
					DBG_PRINTF("here\n");
					//continue;
					cout << "error in io_accept::handle_pkt_in (recv -1) error number : "<<WSAGetLastError()<< endl;
					PAUSE
					exit(1);
					return RET_SOCK_ERROR;
					//PAUSE
				...
				printf("sock closed\n");
				cout << "error in io_accept::handle_pkt_in (recv 0) error number : "<<WSAGetLastError()<< endl;
				PAUSE
				exit(1);
					//PAUSE
				return RET_SOCK_ERROR;
		...
		if (!chunk_ptr) {
			cout << "error in io_accept::handle_pkt_in (new chunk failed) error number : "<<WSAGetLastError()<< endl;
			PAUSE
			exit(1);
			return RET_SOCK_ERROR;
		}
		...
		while (1) {
			...
			if (recv_byte < 0) {
				...
				if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
					...
					cout << "haha5" << endl;
					cout << "error in io_accept::handle_pkt_in (recv -1 payload) error number : "<<WSAGetLastError()<< endl;
					PAUSE
					exit(1);
					//PAUSE
					return RET_SOCK_ERROR;
				...
				printf("sock closed\n");
				cout << "error in io_accept::handle_pkt_in (recv 0 payload) error number : "<<WSAGetLastError()<< endl;
				PAUSE
				exit(1);
					//PAUSE
				return RET_SOCK_ERROR;
...
//			fprintf(_peer_communication_ptr->peer_com_log,"CHNK_CMD_ROLE in io_accept::handle_pkt_in\n",new_fd);
			...
			*/
			...
			if(map_fd_info_iter != _peer_communication_ptr->map_fd_info.end()){
				printf("fd already in map_fd_info in io_accept::handle_pkt_in\n");
				PAUSE
				exit(1);
			}
			else{
					...
					if((session_id_candidates_set_iter->second->manifest == role_protocol_ptr->manifest)&&(session_id_candidates_set_iter->second->role == role_protocol_ptr->flag)){
						for(int i=0;i<session_id_candidates_set_iter->second->peer_num;i++){
							if(session_id_candidates_set_iter->second->list_info->level_info[i]->pid == role_protocol_ptr->send_pid){
								...
								map_fd_info_iter = _peer_communication_ptr->map_fd_info.find(new_fd);
								if(map_fd_info_iter == _peer_communication_ptr->map_fd_info.end()){
									printf("error : cannot new io_accept::handle_pkt_in\n");
									PAUSE
									exit(1);
								}
...
//					fprintf(_peer_communication_ptr->peer_com_log,"fd : %d cannot find list information in io_accept::handle_pkt_in\n",new_fd);
					...
					/*map_fd_unknown_iter = map_fd_unknown.find(new_fd);
					if(map_fd_unknown_iter != map_fd_unknown.end()){
						printf("this fd : %d already in map_fd_unknown in io_accept::handle_pkt_in\n",new_fd);
						exit(1);
					}
					else{
						map_fd_unknown[new_fd] = 1;
					}*/
...
int io_accept::handle_pkt_out(int sock)
{
	...
	*/
	printf("error : place in io_accept::handle_pkt_out\n");
	PAUSE
	exit(1);
...
{
	cout << "error in io_accept handle_pkt_error error number : "<<WSAGetLastError()<< endl;
	PAUSE
	exit(1);
}
...
void io_accept::handle_sock_error(int sock, basic_class *bcptr){
	cout << "error in io_accept handle_sock_error error number : "<<WSAGetLastError()<< endl;
	PAUSE
	exit(1);
}